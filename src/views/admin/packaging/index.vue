  ...
  <div style="margin-left: 20px; margin-right: 20px; margin-bottom: 20px;">
    <div>
      <div style="display: flex; flex-direction: row; width: 100%; justify-content: flex-start; margin-top: 22px; align-items: center; margin-bottom: 22px;">
        ...
        @click="//()=>{
        //  this.$refs.table.toggleRowExpansion(this.filteredUsers[this.openedRow], true)
        //}
        "
        ...
        <el-table
          ...
          <el-table-column 
            ...
            <template slot-scope="scope">
              ...
              <el-table
                :default-sort = "{prop: 'createdAt', order: 'descending'}"
                :data="scope.row.boxes" v-loading.body="scope.row.loading" element-loading-text="Loading" fit highlight-current-row>
                <el-table-column
                  align="center"
                  width="80"
                  label="Actions">
                  <template slot-scope="scope">
                    ...
                    size="mini"
                    icon="el-icon-delete"
                    @click="showDelete2(scope.$index, scope.row)"></el-button>
                  </template>
                </el-table-column>
                <el-table-column 
                  prop="expand"
                ...
                <el-table-column
                  ...
                  width="130">
                  <template slot-scope="scope">
                    <img width="120" height="120" :src="scope.row.qrCode"/>
                  </template>
                </el-table-column>
                <el-table-column
                  align="center"
                  ...
                  width="65">
                  <template slot-scope="scope">
                    <el-button type="primary" size="small" icon="el-icon-download" @click="()=>downloadAsPNG(scope.row)"></el-button>
                  </template>
                </el-table-column>
                <el-table-column
                  prop="status"
                  label="Status">
                  <template slot-scope="scope">
                    <div :style="scope.row.status === 'Pending' ? 'border-color: #E6A23C;' : scope.row.status === 'Scanned' ? 'border-color: #67C23A;' : 'border-color: #909399;'" style="display: flex; justify-content: center; align-items: center; width: 80px; height: 28px; background-color: #fff; border-radius: 22px; border: 1px solid;">
                      <span :style="scope.row.status === 'Pending' ? 'color: #E6A23C;' : scope.row.status === 'Scanned' ? 'color: #67C23A;' : 'color: #909399;'">{{scope.row.status}}</span>
                    </div>
                  </template>
                </el-table-column>
              </el-table>
          ...
          <el-table-column
            ...
            label="Export">
            <template slot-scope="scope">
              <el-button type="primary" size="small" icon="el-icon-download" @click="()=>downloadAsPDF(scope.row._id, scope.row.billNo)">PDF</el-button>
              <el-button type="primary" size="small" icon="el-icon-download" @click="()=>downloadAsQRCode(scope.row._id, scope.row.billNo)">QR Code</el-button>
            </template>
          </el-table-column>
        </el-table>
    ...
    <el-dialog title="Complete Tasks" :visible.sync="userUpdate" :before-close="resetForm">
      <el-input-number style="width: 100%;" type="number" v-model="currUser2.quantity" placeholder="Amount of Boxes"></el-input-number>
      <div v-for="(item, index) in currUser2.newboxes" :key="index" style="display: flex; position: relative; flex-direction: column; justify-content: flex-start; align-items: flex-start; padding: 20px 20px 20px 20px; background-color: #fff; width: 100%; border-bottom: 1px solid #ccc;">
        <el-form ref="currUser" :model="item" label-width="240px">
          <el-form-item label="Product">
            <el-select style="width: 100%;" v-model="item.itemIndex">
          ...
          <el-form-item label="Amount">
            ...
            <el-input-number :min="1" :max="item.itemIndex !== undefined ? currUser.items[item.itemIndex].maxCount - currUser.items[item.itemIndex].currentCount : 1" style="width: 180px;" type="number" v-model="item.amount"></el-input-number>
            <span>/ {{item.itemIndex !== undefined ? currUser.items[item.itemIndex].maxCount - currUser.items[item.itemIndex].currentCount : 1}}</span>
          </el-form-item>
          <el-button @click="()=>{
            currUser2.newboxes.splice(index, 1)
          }" size="small" icon="el-icon-delete" style="margin-left: 240px;" type="danger">Remove</el-button>
        </el-form>
      </div>
      <el-button @click="addItem" size="small" icon="el-icon-plus" type="primary" style="margin-top: 20px;">Add</el-button>
    ...
    <el-dialog title="Delete Packing List" :visible.sync="userDelete" :before-close="resetForm">
      <span>Are you sure you would like to delete this packing list?</span>
      <span slot="footer" class="dialog-footer">
        <el-button @click="resetForm">Cancel</el-button>
        <el-button v-loading="btnloading" type="primary" @click="onSubmit3">Confirm</el-button>
      </span>
    </el-dialog>
    <el-dialog title="Delete Packing List Box" :visible.sync="userDelete2" :before-close="resetForm">
      <span>Are you sure you would like to delete this packing list box?</span>
      <span slot="footer" class="dialog-footer">
        <el-button @click="resetForm">Cancel</el-button>
...
import { getPackingLists, populatePackingLists, addPackingList, pdfPackingList, qrcodePackingList, deletePackingList, deletePackingListBox } from '@/api/packinglists'
...
export default {
  ...
  methods: {
    ...
    showDelete2(index, row) {
      this.currId = row._id
      this.userDelete2 = true
    },
    resetForm() {
      this.currUser = {}
      this.currUser2 = {
    ...
    async downloadAsPDF(id, billNo) {
      ...
      const resp = await pdfPackingList(id)
      var link = document.createElement('a')
      link.href = URL.createObjectURL(new Blob([resp]))
      var fileName = 'pl-' + billNo + '.pdf'
      link.download = fileName
      link.click()
      loading.close()
    },
    async downloadAsQRCode(id, billNo) {
      const loading = this.$loading({
        lock: true,
        text: 'Loading',
        spinner: 'el-icon-loading',
      ...
      const resp = await qrcodePackingList(id)
      var link = document.createElement('a')
      link.href = URL.createObjectURL(new Blob([resp]))
      var fileName = 'pl-' + billNo + '.pdf'
      link.download = fileName
    ...
    async onSubmit2() {
      this.btnloading = true
      const data = {
        items: this.currUser2.newboxes,
        quantity: this.currUser2.quantity
      }
      try {
        await addPackingList(this.currUser._id, data)
        this.btnloading = false
        this.userUpdate = false
        await this.fetchData()
      } catch (err) {
    ...
    async onSubmit3() {
      this.btnloading = true
      try {
        await deletePackingList(this.currId)
      } catch (err) {
        // console.log(err)
      }
      this.btnloading = false
    ...
    async onSubmit4() {
      this.btnloading = true
      try {
        await deletePackingListBox(this.currId)
      } catch (err) {
        // console.log(err)
      }
      this.btnloading = false
    ...
    async addItem() {
      this.currUser2.newboxes.push({amount: undefined, itemIndex: undefined})
    },
    ...
    // async generateQRCode(row, expanded) {
    //   row.loading = true
    //   if (expanded.find((v) => v._id === row._id)) {
    //     for (var i in row.entries) {
    //       const resp = await QRCode.toDataURL(row.entries[i]._id)
    //       row.entries[i].qrCode = resp
    //     }
    //   }
    //   row.loading = false
    // },
    async downloadAsPNG(row) {
      const a = document.createElement('a')
      let json = {_id: row._id, billNo: row.billNo }
      a.href = await QRCode.toDataURL(JSON.stringify(json))
      a.download = row.billNo + '.png'
      a.click()
    },
    test() {
      this.currUser2 = {
    ...
    async fetchData() {
      ...
      const resp = await getPackingLists(this.limit, this.page, this.search, dates, this.dateType)
      this.users = resp.data
      this.total = resp.length
      this.filteredUsers = this.users
      this.loading = false
      
      // if (this.openedRow !== null) {
      //  this.$refs.table.toggleRowExpansion(this.filteredUsers[this.openedRow], true)
      // }
    }
  },
