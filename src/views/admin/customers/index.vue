  ...
  <div class="constantScroll" style="margin-left: 20px; margin-right: 20px; margin-bottom: 20px;">
    <div>
      <div style="display: flex; flex-direction: row; width: 100%; justify-content: flex-start; margin-top: 22px; align-items: center; margin-bottom: 22px;">
        <el-button @click="addProduct" type="success">Add Customer</el-button>
      </div>
      <div style="display: flex; flex-direction: row; width: 100%; justify-content: flex-start; margin-top: 22px; align-items: center; margin-bottom: 22px;">
      ...
      <div style="display: flex; flex-direction: row; width: 100%; justify-content: start; align-items: center; margin-bottom: 12px;">
        ...
        <el-radio v-for="(item, idx) in filterOptions" :key="idx" @change="()=>fetchData()" v-model="radio" :label="item.code">{{ item.code }}</el-radio>
      </div>
    ...
    <el-dialog title="Add Customer" :visible.sync="userCreate" :before-close="resetForm">
    ...
    <el-dialog title="Edit Customer" :visible.sync="userUpdate" :before-close="resetForm">
    ...
    <el-dialog title="Delete Customer" :visible.sync="userDelete" :before-close="resetForm">
      <span>Are you sure you would like to delete this customer?</span>
      <span slot="footer" class="dialog-footer">
        <el-button @click="resetForm">Cancel</el-button>
        <el-button v-loading="btnloading" type="primary" @click="onSubmit3">Confirm</el-button>
      </span>
    </el-dialog>
    <el-dialog title="Duplicate Customer" :visible.sync="userDuplicate" :before-close="resetForm">
      <span>Are you sure you would like to duplicate this customer?</span>
      <span slot="footer" class="dialog-footer">
        <el-button @click="resetForm">Cancel</el-button>
...
<script>
import { getCustomers, createCustomer, duplicateCustomer, updateCustomer, deleteCustomer } from '@/api/customer'
...
export default {
  ...
  methods: {
    ...
    async showUpdate(index, row) {
      ...
      const resp = await findCustomercodes()
    ...
    async onSubmit2() {
      ...
      try {
        await updateCustomer(this.currUser._id, data)
      } catch (err) {
        // console.log(err)
      }
      this.btnloading = false
    ...
    async onSubmit3() {
      this.btnloading = true
      try {
        await deleteCustomer(this.currId)
      } catch (err) {
        // console.log(err)
      }
      this.btnloading = false
    ...
    async onSubmit4() {
      this.btnloading = true
      try {
        await duplicateCustomer(this.currId)
      } catch (err) {
        // console.log(err)
      }
      this.btnloading = false
    ...
    async fetchData() {
      ...
      const resp = await getCustomers(this.radio, this.limit, this.page, this.search, dates, this.dateType)
      this.users = resp.data
      this.total = resp.length
      this.filteredUsers = this.users
      const resp2 = await findCustomercodes()
      this.filterOptions = resp2.data
      this.loading = false
    }
  },
  async mounted() {
