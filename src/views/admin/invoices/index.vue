<template>
  <div style="margin-left: 20px; margin-right: 20px; margin-bottom: 20px;">
    <div>
      ...
      <!-- <div style="display: flex; flex-direction: row; width: 100%; justify-content: flex-start; margin-top: 22px; align-items: center; margin-bottom: 22px;">
        <el-button @click="addProduct" type="success" icon="el-icon-plus">Add Invoice</el-button>
      </div> -->
      <div style="display: flex; flex-direction: column; justify-content: flex-start; align-items: center;">
        <el-table
          ...
          <el-table-column
            ...
            width="110">
            <template slot-scope="scope">
              <span>INV/{{ scope.row.billNo }}</span>
            </template>
          </el-table-column>
          <el-table-column
            prop="date"
    ...
    <el-dialog title="Add Invoice" :visible.sync="userCreate" :before-close="resetForm">
    ...
    <el-dialog title="Update Invoice" :visible.sync="userUpdate" :before-close="resetForm">
    ...
    <el-dialog title="Delete Invoice" :visible.sync="userDelete" :before-close="resetForm">
      <span>Are you sure you would like to delete this invoice?</span>
      <span slot="footer" class="dialog-footer">
        <el-button @click="resetForm">Cancel</el-button>
...
import { getInvoices, createInvoice, updateInvoice, deleteInvoice, pdfInvoice } from '@/api/invoices'
...
export default {
  ...
  methods: {
    ...
    async downloadAsPDF(id, billNo) {
      ...
      const resp = await pdfInvoice(id)
      var link = document.createElement('a')
      link.href = URL.createObjectURL(new Blob([resp]))
      var fileName = 'inv-' + billNo + '.pdf'
    ...
    async onSubmit2() {
      ...
      try {
        await updateInvoice(this.currUser._id, data)
      } catch (err) {
        // console.log(err)
      }
      this.btnloading = false
    ...
    async onSubmit3() {
      this.btnloading = true
      try {
        await deleteInvoice(this.currId)
      } catch (err) {
        // console.log(err)
      }
      this.btnloading = false
    ...
    async fetchData() {
      ...
      const resp = await getInvoices(this.limit, this.page, this.search, dates, this.dateType)
