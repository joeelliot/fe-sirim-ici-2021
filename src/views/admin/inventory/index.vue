...
export default {
  ...
  methods: {
    ...
    async downloadAsPDF() {
      ...
      const resp = await pdfProducts('updatedAt', this.limit, this.page, this.search, dates, this.dateType, 'Raw')
      var link = document.createElement('a')
      link.href = URL.createObjectURL(new Blob([resp]))
      var fileName = ""
      if (dates) {
        fileName = 'product-raw-' + moment(this.dates[0]).format('YYYYMMDD') + '-' + moment(this.dates[1]).format('YYYYMMDD') + '.pdf'
      } else {
        fileName = 'product-raw-master.pdf'
      }
    ...
    async fetchData() {
      ...
      const resp = await getProducts('updatedAt', this.limit, this.page, this.search, dates, this.dateType, 'Raw')
