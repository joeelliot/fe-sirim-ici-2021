<template>
  <div style="margin-left: 20px; margin-right: 20px; margin-bottom: 20px;">
    <div>
      ...
      <!-- <div style="display: flex; flex-direction: row; width: 100%; justify-content: flex-start; margin-top: 22px; align-items: center; margin-bottom: 22px;">
        <el-button @click="addProduct" type="success" icon="el-icon-plus">Add DeliveryOrder</el-button>
      </div> -->
    ...
    <el-dialog title="Add Delivery Order" :visible.sync="userCreate" :before-close="resetForm">
    ...
    <el-dialog title="Update Delivery Order" :visible.sync="userUpdate" :before-close="resetForm">
    ...
    <el-dialog title="Delete Delivery Order" :visible.sync="userDelete" :before-close="resetForm">
      <span>Are you sure you would like to delete this delivery order?</span>
      <span slot="footer" class="dialog-footer">
        <el-button @click="resetForm">Cancel</el-button>
...
import { getDeliveryOrders, createDeliveryOrder, updateDeliveryOrder, deleteDeliveryOrder, pdfDeliveryOrder } from '@/api/deliveryorders'
...
export default {
  ...
  methods: {
    ...
    async downloadAsPDF(id, billNo) {
      ...
      const resp = await pdfDeliveryOrder(id)
      var link = document.createElement('a')
      link.href = URL.createObjectURL(new Blob([resp]))
      var fileName = 'do-' + billNo + '.pdf'
    ...
    async onSubmit2() {
      ...
      try {
        await updateDeliveryOrder(this.currUser._id, data)
      } catch (err) {
        // console.log(err)
      }
      this.btnloading = false
    ...
    async onSubmit3() {
      this.btnloading = true
      try {
        await deleteDeliveryOrder(this.currId)
      } catch (err) {
        // console.log(err)
      }
      this.btnloading = false
    ...
    async fetchData() {
      ...
      const resp = await getDeliveryOrders(this.limit, this.page, this.search, dates, this.dateType)
