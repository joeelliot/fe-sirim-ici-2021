<template>
  <div style="margin-left: 20px; margin-right: 20px; margin-bottom: 20px;">
    <div>
      ...
      <div style="display: flex; flex-direction: column; justify-content: flex-start; align-items: center;">
        <el-table
          ...
          <el-table-column
            v-if="this.$store.state.user.role !== 'authenticated'"
            align="center"
            width="80"
            label="Actions">
          ...
          <el-table-column
            ...
            width="320">
            <template slot-scope="scope">
              <span>{{ scope.row.agent.billNo }} | {{ scope.row.agent.name }} | {{ scope.row.agent.tel }}</span>
            </template>
          </el-table-column>
          <el-table-column
            prop="date"
    ...
    <el-dialog title="Delete Delivery Agent" :visible.sync="userDelete" :before-close="resetForm">
      <span>Are you sure you would like to delete this delivery agent?</span>
      <span slot="footer" class="dialog-footer">
        <el-button @click="resetForm">Cancel</el-button>
...
import { getDeliveryAgents, addDeliveryAgent, deleteDeliveryAgent } from '@/api/deliveryagents'
...
export default {
  ...
  methods: {
    ...
    async downloadAsPDF(id, billNo) {
      ...
      const resp = await pdfDeliveryAgent(id)
    ...
    async onSubmit2() {
      ...
      try {
        await addDeliveryAgent(this.currUser._id, data)
      } catch (err) {
        // console.log(err)
      }
      this.btnloading = false
    ...
    async onSubmit3() {
      this.btnloading = true
      try {
        await deleteDeliveryAgent(this.currId)
      } catch (err) {
        // console.log(err)
      }
      this.btnloading = false
    ...
    async fetchData() {
      ...
      const resp = await getDeliveryAgents(this.limit, this.page, this.search, dates, this.dateType)
