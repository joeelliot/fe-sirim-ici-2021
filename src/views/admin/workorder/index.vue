<template>
  <div style="margin-left: 20px; margin-right: 20px; margin-bottom: 20px;">
    <div>
      ...
      <!-- <div style="display: flex; flex-direction: row; width: 100%; justify-content: flex-start; margin-top: 22px; align-items: center; margin-bottom: 22px;">
        <el-button @click="addProduct" type="success" icon="el-icon-plus">Add WorkOrder</el-button>
      </div> -->
      <div style="display: flex; flex-direction: column; justify-content: flex-start; align-items: center;">
        <el-table
          ...
          <el-table-column
            ...
            width="110">
            <template slot-scope="scope">
              <span>WO/{{ scope.row.billNo }}{{ scope.row.workOrderType }}</span>
            </template>
          </el-table-column>
          <el-table-column
            prop="date"
          ...
          <el-table-column
            ...
            width="110">
            <template slot-scope="scope">
              <el-button type="primary" icon="el-icon-download" @click="()=>downloadAsPDF(scope.row._id, scope.row.billNo, scope.row.workOrderType)">PDF</el-button>
            </template>
          </el-table-column>
        </el-table>
    ...
    <el-dialog title="Add Work Order" :visible.sync="userCreate" :before-close="resetForm">
    ...
    <el-dialog title="Update Work Order" :visible.sync="userUpdate" :before-close="resetForm">
    ...
    <el-dialog title="Delete Work Order" :visible.sync="userDelete" :before-close="resetForm">
      <span>Are you sure you would like to delete this work order?</span>
      <span slot="footer" class="dialog-footer">
        <el-button @click="resetForm">Cancel</el-button>
...
import { getWorkOrders, createWorkOrder, updateWorkOrder, deleteWorkOrder, pdfWorkOrder } from '@/api/workorders'
...
export default {
  ...
  methods: {
    ...
    async downloadAsPDF(id, billNo, workOrderType) {
      ...
      const resp = await pdfWorkOrder(id)
      var link = document.createElement('a')
      link.href = URL.createObjectURL(new Blob([resp]))
      var fileName = 'wo-' + billNo + workOrderType + '.pdf'
    ...
    async onSubmit2() {
      ...
      try {
        await updateWorkOrder(this.currUser._id, data)
      } catch (err) {
        // console.log(err)
      }
      this.btnloading = false
    ...
    async onSubmit3() {
      this.btnloading = true
      try {
        await deleteWorkOrder(this.currId)
      } catch (err) {
        // console.log(err)
      }
      this.btnloading = false
    ...
    async fetchData() {
      ...
      const resp = await getWorkOrders(this.limit, this.page, this.search, dates, this.dateType)
