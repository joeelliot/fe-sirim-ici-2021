<template>
  <div class="constantScroll" style="margin-left: 20px; margin-right: 20px; margin-bottom: 20px;">
    <div>
      ...
      <!-- <div style="display: flex; flex-direction: row; width: 100%; justify-content: flex-start; margin-top: 22px; align-items: center; margin-bottom: 22px;">
        <el-button @click="addProduct" type="success" icon="el-icon-plus">Add Cash Sale</el-button>
      </div> -->
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              ...
              <el-button type="primary" icon="el-icon-download" @click="()=>downloadAsReceipt(scope.row._id, scope.row.billNo)">Receipt</el-button>
            </template>
          </el-table-column>
          <el-table-column
            prop="export"
            ...
            width="210">
            <template slot-scope="scope">
              <el-button type="primary" @click="()=>nextStage(scope.row._id)">Create Delivery Orders</el-button>
            </template>
          </el-table-column>
        </el-table>
    ...
    <el-dialog title="Add Cash Sale" :visible.sync="userCreate" :before-close="resetForm">
    ...
    <el-dialog title="Update Cash Sale" :visible.sync="userUpdate" :before-close="resetForm">
      <el-form ref="currUser" :model="currUser" label-width="240px">
        ...
        <el-form-item label="Payment Method">
          <el-input
            ...
            class="el-input-size" v-model="currUser.paymentMethod"></el-input>
        </el-form-item>
        <el-form-item label="Code">
        ...
        <el-form-item label="Deposit">
          <el-input class="el-input-size" v-model="currUser.deposit"></el-input>
        </el-form-item>
        <el-form-item label="SST">
    ...
    <el-dialog title="Delete Cash Sale" :visible.sync="userDelete" :before-close="resetForm">
      <span>Are you sure you would like to delete this cash sale?</span>
      <span slot="footer" class="dialog-footer">
        <el-button @click="resetForm">Cancel</el-button>
...
import { getCashSales, createCashSale, updateCashSale, deleteCashSale, pdfCashSale, receiptCashSale } from '@/api/cashsales'
...
export default {
  ...
  methods: {
    ...
    async downloadAsPDF(id, billNo) {
      ...
      const resp = await pdfCashSale(id)
      var link = document.createElement('a')
      link.href = URL.createObjectURL(new Blob([resp]))
      var fileName = 'cs-' + billNo + '.pdf'
      link.download = fileName
      link.click()
      loading.close()
    },
    async downloadAsReceipt(id, billNo) {
      const loading = this.$loading({
        lock: true,
        text: 'Loading',
        spinner: 'el-icon-loading',
      ...
      const resp = await receiptCashSale(id)
      var link = document.createElement('a')
      link.href = URL.createObjectURL(new Blob([resp]))
      var fileName = 'cs-' + billNo + '.pdf'
      link.download = fileName
    ...
    async onSubmit2() {
      ...
      const data = {
        ...
        paymentMethod: this.currUser.paymentMethod,
        code: this.currUser.code,
        bulkDiscount: this.currUser.bulkDiscount,
        serviceTax: this.currUser.serviceTax,
        items: this.currUser.items,
        deposit: this.currUser.deposit
      }
      try {
        await updateCashSale(this.currUser._id, data)
      } catch (err) {
        // console.log(err)
      }
      this.btnloading = false
    ...
    async onSubmit3() {
      this.btnloading = true
      try {
        await deleteCashSale(this.currId)
      } catch (err) {
        // console.log(err)
      }
      this.btnloading = false
    ...
    async fetchData() {
      ...
      const resp = await getCashSales(this.limit, this.page, this.search, dates, this.dateType)
