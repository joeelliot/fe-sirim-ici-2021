<template>
  <div class="constantScroll" style="margin-left: 20px; margin-right: 20px; margin-bottom: 20px;">
      ...
      <div style="display: flex; flex-direction: column; justify-content: flex-start; align-items: center;">
        <el-table
          ...
          <el-table-column
            ...
            width="500">
            <template slot-scope="scope">
              <el-button type="primary" @click="()=>showCashOrder(scope.$index, scope.row)">Create Cash Sales</el-button>
              <el-button type="primary" @click="()=>showWorkOrder(scope.$index, scope.row)">Create Work Order</el-button>
              <el-button type="primary" @click="()=>nextStage(scope.row._id)">Create Invoice</el-button>
            </template>
          </el-table-column>
        </el-table>
    ...
    <el-dialog title="Add Sales Order" :visible.sync="userCreate" :before-close="resetForm">
    ...
    <el-dialog title="Update Sales Order" :visible.sync="userUpdate" :before-close="resetForm">
        ...
        <el-form-item label="Products">
          <div style="display: flex; align-items: flex-start; justify-content: column; flex-direction: column; width: 100%;">
            <div v-for="(item, key) in currUser.items" :key="key" style="display: flex; position: relative; flex-direction: column; justify-content: flex-start; align-items: flex-start; padding: 20px 20px 20px 20px; background-color: #fff; width: 100%; border-bottom: 1px solid #ccc;">
              <el-form :ref="'currInstaller' + key" :model="item">
                <el-form-item prop="code" label="Code">
                  <!-- <el-input class="el-input-size" v-model="item.code"></el-input> -->
                  <el-select
                    style="width: 100%;"
    ...
    <el-dialog title="Delete Sales Order" :visible.sync="userDelete" :before-close="resetForm">
      <span>Are you sure you would like to delete this sales order?</span>
      <span slot="footer" class="dialog-footer">
        <el-button @click="resetForm">Cancel</el-button>
        <el-button v-loading="btnloading" type="primary" @click="onSubmit3">Confirm</el-button>
      </span>
    </el-dialog>
    <el-dialog title="Create Cash Order" :visible.sync="userCashOrder" :before-close="resetForm">
      <el-form ref="currUser" :model="amount" label-width="240px">
        <el-form-item label="Price (RM)">
          <el-input class="el-input-size" v-model="amount"></el-input> / RM {{currUser.unpaidBalance}}
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <div style="display: flex; justify-content: center; align-items: center; font-size: 21px; color: #eee; cursor: pointer; position: absolute; left: 20px; width: 50px; height: 50px;" @click="test">
        ...
        <el-button @click="resetForm">Cancel</el-button>
        <el-button v-loading="btnloading" type="primary" @click="nextStage2">Confirm</el-button>
      </span>
      <!-- <div v-for="(item, index) in currUser2.items" :key="index" style="display: flex; position: relative; flex-direction: column; justify-content: flex-start; align-items: flex-start; padding: 20px 20px 20px 20px; background-color: #fff; width: 100%; border-bottom: 1px solid #ccc;">
        <el-form ref="currUser" :model="item" label-width="240px">
          ...
          <el-form-item label="Amount">
            ...
            <el-input-number :min="1" :max="item.itemIndex !== undefined ? Number(currUser.items[item.itemIndex].quantity) - currUser.items[item.itemIndex].currentCount : 1" style="width: 180px;" type="number" v-model="item.quantity"></el-input-number>
            <span>/ {{item.itemIndex !== undefined ? Number(currUser.items[item.itemIndex].quantity) - currUser.items[item.itemIndex].currentCount : 1}}</span>
          </el-form-item>
          <el-button @click="()=>{
            currUser2.items.splice(index, 1)
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="resetForm">Cancel</el-button>
        <el-button v-loading="btnloading" type="primary" :disabled="currUser2.items.length == 0" @click="nextStage2">Confirm</el-button>
      </span> -->
    </el-dialog>
    <el-dialog title="Create Work Order" :visible.sync="userWorkOrder" :before-close="resetForm">
      <el-form ref="currUser" :model="amount" label-width="240px">
        <el-form-item label="Work Order Type">
          <el-select v-model="workOrderType">
            <el-option label="Lelaki" value="L">Lelaki</el-option>
            <el-option label="Perempuan" value="P">Perempuan</el-option>
            <el-option label="Kanak-Kanak" value="K">Kanak-Kanak</el-option>
            <el-option label="Umum" value="U">Umum</el-option>
          </el-select>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        ...
        <el-button @click="resetForm">Cancel</el-button>
        <el-button v-loading="btnloading" type="primary" :disabled="workOrderType === ''" @click="nextStage3">Confirm</el-button>
      </span>
    </el-dialog>
...
import { createCashSale } from '@/api/cashsales'
import { getSalesOrders, createSalesOrder, updateSalesOrder, deleteSalesOrder, pdfSalesOrder } from '@/api/salesorders'
...
export default {
  ...
  methods: {
    ...
    showCashOrder(index, row) {
      this.currId = row._id
      this.currUser = row
      this.userCashOrder = true
    },
    showWorkOrder(index, row) {
      this.currId = row._id
      this.currUser = row
      this.userWorkOrder = true
    },
    resetForm() {
      this.currUser = {}
      this.currUser2 = {
      ...
      this.userCashOrder = false
      this.userWorkOrder = false
    },
    ...
    async nextStage2() {
      try {
        // console.log(this.currUser.items)
        // console.log(this.currUser2.items)
        await createCashSale({
          salesorderId: this.currId,
          // items: this.currUser2.items
          amount: this.amount
        })
        this.$router.push({ name: 'cashsales' })
      } catch (err) {
        // console.log(err)
      }
    },
    ...
    async downloadAsPDF(id, billNo) {
      ...
      const resp = await pdfSalesOrder(id)
    ...
    async onSubmit2() {
      ...
      try {
        await updateSalesOrder(this.currUser._id, data)
      } catch (err) {
        // console.log(err)
      }
      this.btnloading = false
    ...
    async onSubmit3() {
      this.btnloading = true
      try {
        await deleteSalesOrder(this.currId)
      } catch (err) {
        // console.log(err)
      }
      this.btnloading = false
    ...
    async addItem() {
      this.currUser2.items.push({quantity: undefined, itemIndex: undefined})
    },
    async addItem2() {
    ...
    async fetchData() {
      ...
      const resp = await getSalesOrders(this.limit, this.page, this.search, dates, this.dateType)
